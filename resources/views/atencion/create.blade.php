...
@section('titulo_seccion','Nueva Atención')


@section('breadcrumb')
<li class="breadcrumb-item active">Registro Atención</li>
@endsection
    ...
    <div class="col-md-8">
        <div class="card card-danger">
            <div class="card-header">
                <h3 class="card-title">Formulario Atención</h3>
              </div>
              @include('atencion.forms.formAtencion')
              {{--<form method="POST" action="{{ route('atencion.store') }}">
                <div class="card-body">
                @csrf
                    ...
                    <div class="form-group">
                        ...
                        <select name="select_ficha" id="select_ficha" class="form-control @error('select_ficha') is-invalid @enderror">
                            ...
                            @foreach($ficha as $f)
                            <option value="{{$f->id}}"
                            {{ (int) old('select_ficha') === $f->id ? 'selected' : ''}}>{{$f->nombre_paciente}}</option>
                            @endforeach
                        </select>
                            @error('select_ficha')
                    ...
                    <div class="form-group">
                        ...
                        <select name="select_medico" id="select_medico" class="form-control @error('select_medico') is-invalid @enderror">
                            ...
                            @foreach($medico as $m)
                            <option value="{{$m->id}}"
                            {{ (int) old('select_medico') === $m->id ? 'selected' : ''}}>{{$m->nombre_medico}}</option>
                            @endforeach
                        </select>
                            @error('select_medico')
                                <span class="error invalid-feedback">{{ $message }}</span>
                    ...
                    <div class="form-group">
                        ...
                        <textarea
                            ...
                            cols="30"
                            rows="05"
                            style="resize : none;">{{ old('sintomas_paciente') ?: "" }}
                        </textarea>
                    ...
                    <div class="form-group">
                        ...
                        <textarea
                            ...
                            cols="30"
                            rows="05"
                            style="resize : none;">{{ old('tratamiento_paciente') ?: "" }}
                        </textarea>
                    ...
                    <div class="form-group">
                        ...
                        <textarea
                            ...
                            cols="30"
                            rows="05"
                            style="resize : none;">{{ old('observaciones_paciente') ?: "" }}
                        </textarea>
                ...
                <div class="card-footer">
                  <button type="submit" class="btn btn-info">Crear Atención</button>
                </div>
              </form>--}}
        </div>
    </div>
