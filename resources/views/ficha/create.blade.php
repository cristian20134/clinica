...
@section('titulo_seccion','Nueva Ficha Paciente')
    ...
    <div class="col-md-8">
        <div class="card card-danger">
            <div class="card-header">
                <h3 class="card-title">Formulario Registro Paciente</h3>
              </div>
            ...
            @include('ficha.forms.formFicha')
              {{--<form method="POST" action="{{ route('ficha.store') }}">
                ...
                @csrf
                    ...
                    <div class="form-group">
                        ...
                        <select name="select_salud" id="select_salud" class="form-control @error('select_salud') is-invalid @enderror">
                            ...
                            <option value="{{$s->id}}"
                            {{ (int) old('select_salud') === $s->id ? 'selected' : ''}}>{{$s->nombre_sistema_salud}}</option>
                            @endforeach
                        </select>
                            @error('select_salud')
                                <span class="error invalid-feedback">{{ $message }}</span>
                    ...
                    <div class="form-group">
                        ...
                        <select name="select_genero" id="select_genero" class="form-control @error('select_genero') is-invalid @enderror">
                            ...
                            @foreach($genero as $g)
                            <option value="{{$g->id}}"
                            {{ (int) old('select_genero') === $g->id ? 'selected' : ''}}>{{$g->nombre_genero}}</option>
                            @endforeach
                        </select>
                            @error('select_genero')
                                <span class="error invalid-feedback">{{ $message }}</span>
                    ...
                    <div class="form-group">
                        ...
                        <select name="select_ciudad" id="select_ciudad" class="form-control @error('select_ciudad') is-invalid @enderror">
                            ...
                            <option value="{{$ci->id}}"
                            {{ (int) old('select_ciudad') === $ci->id ? 'selected' : ''}}>{{$ci->nombre_ciudad}}</option>
                            @endforeach
                        </select>
                            @error('select_ciudad')
                                <span class="error invalid-feedback">{{ $message }}</span>
                ...
                <div class="card-footer">
                  <button type="submit" class="btn btn-info">Crear Nueva Ficha</button>
                </div>
              </form> remplazado por el @include --}}
        </div>
    </div>
